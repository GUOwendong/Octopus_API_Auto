...
class TestProduct:
    ...
    def test_product_flow(self, api_client):
        ...
        assert create_res.get("code") == "ok", f"新增失败: {create_res.get('error', create_res)}"
        sku_id = create_res.get("data", {}).get("firstSkuId")
        ...
        assert rows[0].get("goodsName") == goods_name, f"商品名称不匹配: {rows[0].get('goodsName')}"
        goods_id = rows[0].get("id")
        print(f"✅ 查询成功, goods_id={goods_id}")

        # ===== 3. 下架商品 =====
        delist_res = service.delist(sku_id)
        assert delist_res.get("code") == "ok", f"下架失败: {delist_res.get('error', delist_res)}"
        print("✅ 下架成功")

        # ===== 4. 上架商品 =====
        relist_res = service.relist(sku_id)
        assert relist_res.get("code") == "ok", f"上架失败: {relist_res.get('error', relist_res)}"
        print("✅ 上架成功")
